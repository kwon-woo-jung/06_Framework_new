...
public class ExampleController {
	...
	public String ex1(HttpServletRequest req, Model model) {
		...
		stdt.setStudentNo("12345");
		stdt.setName("홍길동");
		stdt.setAge(22);
		
		model.addAttribute("std", stdt);
		
		// List<Student> 객체 Model을 이용해서 html로 전달
		List<Student> stdLists = new ArrayList<>();
		
		stdLists.add(new Student("11111", "김일번", 20));
		stdLists.add(new Student("22222", "최이번", 23));
		stdLists.add(new Student("33333", "홍삼번", 22));
		
		model.addAttribute("stdList", stdLists);
	...
	public String ex2(Model model) {
		...
		// 접미사 : .html
		// -> src/main/resource/temolates/example/ex2.html
		return "example/ex2"; 
	}
	
	
	@GetMapping("ex3")    // /example/ex3 GET 방식 요청 매핑
	public String ex3(Model models) {
		
		models.addAttribute("key", "제목이야");
		models.addAttribute("query", "검색어야");
		
		models.addAttribute("boardNo", 10);
//		src/main/resources/templates/example/ex3.html
		return "example/ex3";
	}
	...
	@GetMapping("ex4")
	public String ex4(Model models) {
		
		Student stds = new Student("67890", "잠만보", 22);
		
		models.addAttribute("std", stds);
		
		models.addAttribute("num", 1000);
		
		return "example/ex4"; // forward해서 example ex4.html
	}
	
	
	@GetMapping("ex5")
	public String ex5(Model models) {
		
		models.addAttribute("message", "타입리프 + Javascript 사용 연습");
		models.addAttribute("num1", 12345);
